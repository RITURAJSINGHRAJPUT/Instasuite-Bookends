"use client";

import { useCallback, useEffect, useState } from "react";
import { Plus, MessageSquareText, AlertTriangle, X } from "lucide-react";

// Pre-written messages staff can tap to send instantly from the Inbox composer
// (AccountInbox.tsx), scoped per business — Aiko and Capiche each keep their own
// list. Mirrors the Unavailable page's shape: a brand picker, an inline add-form,
// and a flat list with a delete "X" — no modal, consistent with the rest of the app.

type Business = { id: string; name: string };
type QuickReply = {
  id: string;
  business_id: string;
  business_name: string | null;
  title: string;
  message: string;
  created_at: string;
};

const INPUT =
  "rounded-xl border border-[var(--border-strong)] bg-[var(--surface-1)] px-4 py-2.5 text-base text-[var(--text-1)] placeholder:text-[var(--text-6)] focus:border-[var(--accent)] focus:outline-none md:text-sm";

function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <p className="mb-1 text-[11px] font-bold uppercase tracking-wider text-[var(--text-5)]">{label}</p>
      {children}
    </div>
  );
}

export default function QuickRepliesPage() {
  const [businesses, setBusinesses] = useState<Business[]>([]);
  const [businessId, setBusinessId] = useState("");
  const [rows, setRows] = useState<QuickReply[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);
  const [title, setTitle] = useState("");
  const [message, setMessage] = useState("");

  useEffect(() => {
    fetch("/api/businesses")
      .then((r) => (r.ok ? r.json() : []))
      .then((d: Business[]) => {
        const list = Array.isArray(d) ? d.map((b) => ({ id: b.id, name: b.name })) : [];
        setBusinesses(list);
        setBusinessId((cur) => cur || list[0]?.id || "");
      })
      .catch(() => {});
  }, []);

  const load = useCallback(async () => {
    const res = await fetch("/api/quick-replies");
    const data = await res.json();
    if (!res.ok) setError(data?.error || "Couldn't load quick replies.");
    else setRows(Array.isArray(data) ? data : []);
    setLoading(false);
  }, []);
  useEffect(() => {
    load();
  }, [load]);

  const showBiz = businesses.length > 1;
  const visible = businessId ? rows.filter((r) => r.business_id === businessId) : rows;
  const canSubmit = !!title.trim() && !!message.trim() && !!businessId && !saving;

  async function add() {
    if (!canSubmit) return;
    setSaving(true);
    setError(null);
    const res = await fetch("/api/quick-replies", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ business_id: businessId, title: title.trim(), message: message.trim() }),
    });
    const data = await res.json();
    setSaving(false);
    if (!res.ok) return setError(data?.error || "Couldn't add that.");
    setTitle("");
    setMessage("");
    load();
  }

  async function remove(id: string) {
    setRows((prev) => prev.filter((r) => r.id !== id));
    await fetch(`/api/quick-replies/${id}`, { method: "DELETE" });
    load();
  }

  return (
    <div className="mx-auto w-full max-w-3xl px-4 py-6 md:px-8">
      <h1 className="text-xl font-extrabold tracking-tight text-[var(--text-1)]">Quick Replies</h1>
      <p className="text-[13px] text-[var(--text-4)]">
        Pre-written messages staff can tap to send instantly from the Inbox. Each business keeps its
        own list.
      </p>

      <div className="mt-5 rounded-2xl border border-[var(--border)] bg-[var(--panel-bg)] p-4">
        <div className="flex flex-col gap-3">
          {showBiz && (
            <Field label="Brand">
              <select
                value={businessId}
                onChange={(e) => setBusinessId(e.target.value)}
                aria-label="Brand"
                className={`w-full ${INPUT}`}
              >
                {businesses.map((b) => (
                  <option key={b.id} value={b.id}>
                    {b.name}
                  </option>
                ))}
              </select>
            </Field>
          )}
          <Field label="Title">
            <input
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && add()}
              placeholder="e.g. Confirming order"
              className={`w-full ${INPUT}`}
            />
            <p className="mt-1 text-[10px] text-[var(--text-5)]">
              Shown to staff only — never sent to the guest.
            </p>
          </Field>
          <Field label="Message">
            <input
              value={message}
              onChange={(e) => setMessage(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && add()}
              placeholder="e.g. Thanks for waiting — confirming this now!"
              className={`w-full ${INPUT}`}
            />
          </Field>
        </div>
        <button
          onClick={add}
          disabled={!canSubmit}
          className="mt-3 flex w-full items-center justify-center gap-1.5 rounded-xl bg-[var(--accent)] px-4 py-2.5 text-sm font-bold text-[var(--accent-fg)] transition-colors hover:bg-[var(--accent-hover)] disabled:opacity-40"
        >
          <Plus size={14} />
          Add
        </button>
      </div>

      {error && (
        <p className="mt-3 flex items-start gap-2 rounded-lg border border-[var(--danger)]/25 bg-[var(--danger-soft)] px-3 py-2 text-[12px] font-semibold text-[var(--danger)]">
          <AlertTriangle size={13} className="mt-px flex-shrink-0" />
          {error}
        </p>
      )}

      {loading && <p className="mt-4 text-xs text-[var(--text-4)]">Loading…</p>}

      {!loading && visible.length === 0 && (
        <div className="mt-4 rounded-2xl border border-[var(--border)] bg-[var(--panel-bg)] py-10 text-center">
          <MessageSquareText size={20} className="mx-auto text-[var(--text-5)]" />
          <p className="mt-2 text-[12px] font-bold text-[var(--text-1)]">No quick replies yet</p>
          <p className="mt-0.5 text-[11px] text-[var(--text-4)]">Add one above to use it from the Inbox.</p>
        </div>
      )}

      <div className="mt-4 space-y-2">
        {visible.map((r) => (
          <div
            key={r.id}
            className="flex items-center justify-between gap-3 rounded-xl border border-[var(--border)] bg-[var(--panel-bg)] px-4 py-3"
          >
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <p className="truncate text-[13px] font-semibold text-[var(--text-1)]">{r.title}</p>
                {showBiz && r.business_name && (
                  <span className="text-[10px] text-[var(--text-5)]">{r.business_name}</span>
                )}
              </div>
              <p className="mt-0.5 truncate text-[11px] text-[var(--text-4)]">{r.message}</p>
            </div>
            <button
              onClick={() => remove(r.id)}
              aria-label="Delete quick reply"
              className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg text-[var(--text-4)] transition-colors hover:bg-[var(--surface-1)] hover:text-[var(--danger)]"
            >
              <X size={16} />
            </button>
          </div>
        ))}
      </div>
    </div>
  );
}
